  ...
  return (
    <section className="relative py-10 px-4 text-center bg-gradient-to-br from-amber-50 via-orange-50 to-amber-100 overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="flex overflow-x-auto pb-4 mb-6 md:grid md:grid-cols-3 md:gap-6 md:overflow-visible hide-scrollbar">
          {categories.map(category => (
            <div
              key={category.key}
              className="flex-shrink-0 w-64 mx-2 md:mx-0 md:w-auto bg-white rounded-xl shadow-md p-5 flex flex-col items-center transform transition-all duration-300 hover:shadow-lg"
            >
              <div className="text-4xl mb-3">{category.icon}</div>
              <h3 className="text-lg font-bold text-gray-800 mb-2">{category.label}</h3>
              <p className="text-orange-600 font-semibold text-sm mb-4">{category.discount}</p>
              <button
